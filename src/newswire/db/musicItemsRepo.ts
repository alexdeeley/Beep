...
import type { FactLabel, MusicItemType, VerifiedFact } from "../types.js";
  ...
  const result = db
    .prepare(
      `INSERT OR IGNORE INTO music_items
        (watched_artist_id, item_type, headline, summary, fact_label, event_time, event_time_confidence, article_published_at, primary_source_url, source_domains_json, facts_json, discovered_in_run_id, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .run(
