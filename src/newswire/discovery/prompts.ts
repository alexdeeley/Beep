...
export function buildDiscoverySystemPrompt(): string {
  return [
    ...
    'Set releaseFormat and releaseTitle to null when itemType is "news". When itemType is "release", set releaseTitle to the clean',
    'title of the single/album/EP/compilation ONLY (e.g. "Speyside" - not "a new single called Speyside") - this gets used verbatim',
    "in a mechanical \"NEW SINGLE: Artist - Title\" post, so it must be exactly the release's real title, nothing added or paraphrased.",
