    ...
    candidates: {
      ...
      items: {
        ...
        properties: {
          ...
          releaseFormat: { type: "string", enum: ["album", "ep", "compilation"] },
...
export function buildIndustryDiscoverySystemPrompt(): string {
  return [
    ...
    "memory.",
    "Find genuinely major, widely-covered album/EP/compilation releases that came out on or immediately before the current date - the",
    "kind of releases covered by mainstream music press (major-label releases, high-profile independent releases, records generating",
    "real critical or chart attention). Deliberately skip minor, obscure, or unremarkable releases even if technically real - this",
    "should read like a notable-releases roundup, not an exhaustive catalog dump. Never include a single (only full releases: album,",
    "EP, or compilation).",
    'Classify releaseFormat as exactly one of "album" (a full-length), "ep", or "compilation", based on what your sources actually',
    "call it - never guess if genuinely unclear, pick the closest reasonable match.",
...
export function buildIndustryDiscoveryUserPrompt(nowIso: string): string {
  return [
    `Current time: ${nowIso}`,
    "",
    "Find this week's major album/EP/compilation releases across the music industry (New Music Friday style) - any artist, not just",
