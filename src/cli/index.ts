...
program
  .command("publish")
  .description("Upload the rendered image and publish to Instagram (or --dry-run to simulate)")
  .option("--date <YYYY-MM-DD>", "Local publish date (defaults to today)")
  ...
  .action(async (opts) => {
    ...
    if (record.mediaId) console.log(`Instagram media ID: ${record.mediaId}`);
...
program
  ...
  .option("--dry-run", "Run the entire pipeline but never actually publish to Instagram", false)
